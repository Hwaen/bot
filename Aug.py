...
#####저항#####
    if msg == "금강":
        embed=discord.Embed(title="금강", description="상대 실험체를 이동 불가 상태로 만들면 4초 동안 방어가 (20+레벨×5.5) 증가합니다. 방어 상승 효과가 종료되는 즉시 주변 2m에 (레벨×13)의 스킬 피해를 입힙니다. 피해를 입은 대상은 1.5초 동안 이동속도가 50% 감소합니다. (쿨다운 25초)")
        embed.set_thumbnail(url="https://cdn.discordapp.com/attachments/915656436212695080/916339950461550622/Diamond_Shard_.png")
        return embed

    if msg == "불괴":
        embed=discord.Embed(title="불괴", description="무기 스킬 사용 시 3[근거리]/2.5[원거리]초 동안 불괴 상태가 되며 입는 피해가 10 + 레벨×1[근거리]/5 + 레벨×1[원거리]% 감소하고 방해 효과 저항이 (25+방어력×0.15)[근거리]/(18+방어력×0.15)[원거리]% 증가합니다. (쿨다운 20초))")
...
#####파괴#####
    if msg == "취약":
        embed=discord.Embed(title="취약", description="동일 실험체에게 3초 안에 기본 공격 또는 개별의 피해를 입히는 스킬(무기, 아이템 스킬 제외)을 3회 적중 시 (레벨×3)의 고정 피해를 추가로 입히며 5초 동안 대상의 방어력을 (레벨×1.3)% 감소시킵니다. (쿨다운 30초) 기본 공격이 연타로 적용되는 무기, 스킬 및 도트 딜링 스킬은 입히는 연속 피해를 한 번의 피해로 취급합니다. 스킬 사용 후 재사용하는 스킬의 경우는 스킬의 성향에 따라 개별 스킬 취급 여부가 다르며 무기 스킬은 취약 스택을 쌓을 수 없습니다.")
    ...
    if msg == "광분":
        embed=discord.Embed(title="광분", description="무기 스킬 사용 시 7[근거리]/5[원거리]초 동안 공격력이 (10+레벨×1)[근거리]/(5+레벨×0.5)[원거리], 공격 속도가 (30+레벨×1.5)[근거리]/(20+레벨×1.5)[원거리]% 증가하지만 입는 피해도 5% 증가합니다. (쿨다운 30초)")
        embed.set_thumbnail(url="https://cdn.discordapp.com/attachments/916261271630925854/916262946437496853/Frenzy_.png")
        return embed
        
    if msg == "흡혈마":
        embed=discord.Embed(title="흡혈마", description="개별 스킬로 실험체에게 피해를 입힐 때마다 모든 피해 흡혈이 6%씩 상승하며 최대 4번 중첩됩니다. 최대 중첩 시 스킬 증폭이 (레벨1.5)% 증가합니다. 동시에 여러 대상에게 피해를 입힐 경우 피해를 입힌 대상 수만큼 중첩되어 상승합니다. 모든 피해 흡혈은 8초 동안 유지되며 개별 스킬로 피해를 입힐 때마다 갱신됩니다. (쿨다운 5초) 무기 스킬은 흡혈마 스택을 쌓을 수 없습니다.)")
...
#####지원#####
    if msg == "초재생":
        embed=discord.Embed(title="초재생", description="자신의 스킬에 의해 발생하는 모든 보호막 및 회복량이 상시 (20+스킬 증폭×0.2)% 증가합니다. 자신에 의해 보호막 또는 회복 효과를 받은 대상은 4초 동안 공격력이 (레벨×3)만큼 증가합니다. (쿨다운 10초) 휴식, 재생, 흡혈, 음식, 음료를 통한 회복은 적용되지 않습니다.")
    ...
    if msg == "증폭드론":
        embed=discord.Embed(title="증폭드론", description="궁극기 사용 시 4.5초 동안 자신을 따라다니는 증폭 드론이 나타납니다. 증폭 드론은 주변 4m 내 자신을 포함한 아군의 이동 속도를 (5+0.3×레벨)%, 스킬 증폭을 (10+2×레벨)% 증가시킵니다. (쿨다운 45초)")
